import {
  DeleteImageEvent,
  InsertImageEvent
} from "@pie-libs/pie-configure-events";
...
export default class CatalogDemo extends HTMLElement {
  ...
  _addConfigurationPanes() {
    ...
    let panes = this._config.models.map(cfg => {
      let configElement =
        this._configureMap[cfg.element] || "json-configuration";
    ...
    customElements.whenDefined("configuration-pane").then(() => {
      let paneList = this.querySelectorAll("configuration-pane");

      for (var i = 0; i < paneList.length; i++) {
        let p = paneList[i];
        let m = this._config.models.find(
          m => m.id === p.getAttribute("element-id")
        );
        p.model = m;
      }
    });
